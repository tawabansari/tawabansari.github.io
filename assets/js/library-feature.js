(function(){
  ...
  const kinds=['Quran','Roots','Terminology','Articles','Reflection'];let data=[],kind=0,language='en',busy=false;
  ...
  language=section.dataset.featureFixedLanguage || (read('forqan-language','en')==='fa'?'fa':'en');
  function render(){
    let pool=[];for(let attempts=0;attempts<kinds.length;attempts++){pool=data.filter(d=>d.kind===kinds[kind]&&d.lang===language);if(pool.length)break;kind=(kind+1)%kinds.length;}if(!pool.length)return;
    ...
    a.textContent=fa?({Quran:'مطالعه این آیه',Roots:'مطالعه این ریشه',Terminology:'مطالعهٔ مفهوم',Articles:'مطالعه مقاله',Reflection:'مطالعهٔ تأویل'}[item.kind]):({Quran:'Read this verse',Roots:'Explore this root',Terminology:'Explore the concept',Articles:'Read the article',Reflection:'Read the ta’wil'}[item.kind]);content.appendChild(a);
  ...
  fetch('/assets/data/features.json').then(r=>{if(!r.ok)throw Error('features');return r.json();}).then(items=>{data=items;render();}).catch(()=>{next.hidden=true;const languages=section.querySelector('.feature-languages');if(languages)languages.hidden=true;});
}());
