(function(){
  ...
  const kinds=['Quran','Roots','Articles'];let data=[],kind=0,language='en',busy=false;
  ...
  const previous=Number(read('forqan-feature-kind','-1'));kind=Number.isInteger(previous)&&previous>=-1&&previous<3?(previous+1)%3:0;
  ...
  function render(){
    ...
    a.textContent=fa?({Quran:'مطالعه این آیه',Roots:'مطالعه این ریشه',Articles:'مطالعه مقاله'}[item.kind]):({Quran:'Read this verse',Roots:'Explore this root',Articles:'Read the article'}[item.kind]);content.appendChild(a);
  ...
  next.addEventListener('click',()=>change(()=>{kind=(kind+1)%3;}));
  ...
  window.addEventListener('pageshow',event=>{if(event.persisted&&data.length){kind=(kind+1)%3;render();}});
